...
namespace DemonTweaks.Utils
{
    class AssetLoader
    {
        ...
        public static bool GetBrimorakTTTBaseSetting()
        {
            try
            {
                var xks = Path.Combine(ModEntry.Path, @"..\TabletopTweaks-Base\UserSettings\Fixes.json");
    ...
    public static class Helpers
    {
        ...
        public static LocalizedString GetLocalizationElement(string name, string key, string seperator = "-")
        {
            try
            {
                return LocalizationTool.GetString("DemonTweaks." + key + seperator + name);
            }
            catch (Exception ex)
            {
                //Main.Log(ex.Message);
